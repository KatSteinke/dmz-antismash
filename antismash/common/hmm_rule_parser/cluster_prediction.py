...
def find_protoclusters(record: Record, cds_by_cluster_type: Dict[str, Set[str]],
                       rules_by_name: Dict[str, rule_parser.DetectionRule],
                       dmz_cdses: Optional[List[Feature]] = None) -> List[Protocluster]:
    ...
    for cluster_type, cds_names in cds_by_cluster_type.items():
        ...
        for cds in cds_features[1:]:
            if dmz_cdses:
                for dmz_cds in dmz_cdses:
                    if dmz_cds.overlaps_with(core_location):
            ...
            if dmz_cdses:
                for dmz_cds in dmz_cdses:
                    if dmz_cds.overlaps_with(surrounds):
                        if core_location.start < dmz_cds.location.start:
...
def apply_cluster_rules(record: Record, results_by_id: Dict[str, List[HSP]],
                        ...
                        dmz_cdses: Optional[List[Feature]] = None
    ...
    """
        ...
        Args:
            ...
            dmz_cdses: A list of locations a region can't cross
    ...
    for cds_name in cds_with_hits:
        ...
        for rule in rules:
            if rule.cutoff not in info_by_range:
                ...
                if dmz_cdses:
                    for dmz_cds in dmz_cdses:
                        if dmz_cds.overlaps_with(location):
                            if feature < dmz_cds:
...
def detect_protoclusters_and_signatures(record: Record, signature_file: str, seeds_file: str,
                                        ...
                                        demarcation_points: Optional[List] = None,
    ...
    """ Compares all CDS features in a record with HMM signatures and generates
        ...
        Arguments:
            ...
            demarcation_points: a list of integer locations on the genome that no clusters
                               can be formed over
    ...
    dmz_cdses = None # type: Optional[List[Feature]]
    if demarcation_points:
        dmz_cdses = [Feature(FeatureLocation(demarcation_point, demarcation_point + 1), "dmz_feature")
                     for demarcation_point in demarcation_points]
    if dmz_cdses:
        for dmz_cds in dmz_cdses:
    ...
    cds_domains_by_cluster, cluster_type_hits = apply_cluster_rules(record, results_by_id, rules, dmz_cdses)
    ...
    clusters = find_protoclusters(record, cluster_type_hits, rules_by_name, dmz_cdses)
